...
$(function() {
    ...
    d3.csv('data/prepped_data.csv', function(error, data) {
        ...
        var xScale = d3.scaleLog().domain([xMin, xMax]).range([0, drawWidth]);
        ...
        var yScale = d3.scaleLinear().domain([yMin, yMax]).range([drawHeight, 0]);
        ...
        var xAxisLabel = svg.append("g").attr("transform", "translate(" + margin.left + ", " + (drawHeight + margin.top) + ")").call(xAxis)

        // Append a g to your SVG as a y axis label, specifying the 'transform' attribute to position it
        // Make sure to use the `.call` method to render the axis in the label
        var yAxisLabel = svg.append("g").attr("transform", "translate(" + margin.left + ", " + margin.top + ")").call(yAxis)

        // Append a text element to your svg to label your x axis, and place it in the proper location
        var xText = svg.append("text").text("GDP").attr("transform", "translate(" + (margin.left + drawWidth/2) + "," + (margin.top + drawHeight + 40) + ")").attr("class", "title")
        ...
        circles.enter().append("circle").attr("r", 15).attr("cx", function(d){return xScale(d.gdp)})
        .attr("cy", drawHeight)
        .transition()
        .duration(500)
        // 
